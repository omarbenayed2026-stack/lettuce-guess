...
const translations = {
  ...
  fr: {
    ...
    introText: "Bienvenue ! Utilisez Découvrir pour une recette aléatoire. Ouvrez Préférences pour filtrer par budget, style et ingrédients. Appuyez à nouveau pour une autre suggestion (on évite aussi les répétitions).",
  ...
  ar: {
    ...
    introText: "مرحبًا! استخدم «اكتشف» للحصول على وجبة عشوائية. افتح «التفضيلات» للتصفية حسب الميزانية والنوع والمكونات. اضغط مرة أخرى لاقتراح جديد (ونحاول تجنب التكرار).",
    startDiscover: "ابدأ",
    diffEasy: "سهل",
    diffMedium: "متوسط",
    diffHard: "صعب",
...
function applyLanguage(lang) {
  ...
  document.querySelectorAll("[data-i18n]").forEach((el) => {
    ...
    if (!key) return;
    // Enforce "Lettuce Guess" brand name to never translate
    if (key === "brand") { el.textContent = "Lettuce Guess"; return; }
    if (key === "brandPill" && lang !== 'en') { el.textContent = dict[key]; return; }
    if (dict[key] != null) {
      // Preserve emojis if they exist in the HTML but not in translation string
...
function handleDiscover(isDailyPick) {
  ...
  }, 600); // Slightly longer wait for the click animation
}
...
function renderFood(food) {
  ...
  if (els.foodDifficulty) {
    ...
    // Translate the difficulty properly based on the tag
    if (diff === "easy") { els.foodDifficulty.classList.add("badge-green"); els.foodDifficulty.textContent = dict.diffEasy.toUpperCase(); }
    else if (diff === "medium") { els.foodDifficulty.classList.add("badge-gold"); els.foodDifficulty.textContent = dict.diffMedium.toUpperCase(); }
    else if (diff === "hard") { els.foodDifficulty.classList.add("badge-red"); els.foodDifficulty.textContent = dict.diffHard.toUpperCase(); }
    else { els.foodDifficulty.classList.add("badge-blue"); els.foodDifficulty.textContent = "—"; }
  }
